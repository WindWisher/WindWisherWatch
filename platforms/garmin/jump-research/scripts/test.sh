...
garmin_output="${GARMIN_TEST_OUTPUT:-bin/WindWisherJumpResearchTests.prg}"
...
monkeyc -f monkey.jungle -d "${garmin_target_device}" -y "${GARMIN_DEVELOPER_KEY}" --unit-test -o "${garmin_output}"
monkeydo "${garmin_output}" "${garmin_target_device}" -t
