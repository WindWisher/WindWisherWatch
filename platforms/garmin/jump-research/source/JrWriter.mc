...
class JrWriter {
    ...
    function summary(result, duration, source, startMemory, endMemory, operatorReference) {
        var buffer = source.buffer();
        var detector = source.detector();
        // Candidate-window exports reserve the physical app-log budget for the
        // bounded raw window. Aggregate rejected counts remain in the summary;
        // only confirmed trace detail is needed to identify the frozen window.
        var traces = source.mode().equals(JrConstants.MODE_CANDIDATE_WINDOWS) ? detector.confirmedTracesJson() : detector.tracesJson();
        emit("{\"recordType\":\"summary\",\"result\":\"" + result + "\",\"durationMilliseconds\":" + duration + ",\"observedSamples\":" + source.sequence() + ",\"exportedSamples\":" + buffer.size() + ",\"overwrittenOrDroppedSamples\":" + buffer.dropped() + ",\"callbackStatistics\":" + source.stats().toJson() + ",\"detector\":{\"candidateCount\":" + detector.candidateCount() + ",\"confirmedCandidates\":" + detector.confirmed() + ",\"rejectedCandidates\":" + detector.rejected() + ",\"lastExperimentalAirtimeMilliseconds\":" + value(detector.lastAirtime()) + ",\"state\":\"" + detector.stateName() + "\",\"locomotionBounds\":{\"capacity\":8,\"used\":" + detector.locomotionContext().used() + ",\"totalImpacts\":" + detector.locomotionContext().totalImpacts() + "},\"candidateTraces\":" + traces + "},\"operatorReference\":" + operatorReference.toJson() + ",\"memory\":{\"startFreeBytes\":" + startMemory.freeMemory + ",\"endFreeBytes\":" + endMemory.freeMemory + ",\"endUsedBytes\":" + endMemory.usedMemory + "},\"exportPolicy\":\"POST_CAPTURE_TIMER_DRAIN\"}");
